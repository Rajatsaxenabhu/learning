...
from app.api.routes.service.agenttalk import UserAgent
...
class ChatConnection:
    ...
    async def read_question(self) -> str | None:
        data = await self.websocket.receive_json()
        if not isinstance(data, dict):
            return None
        return str(data.get("message", "")).strip() or None
    ...
    async def answer(self, agent: UserAgent, question: str):
        try:
            used = await agent.ask(question, self.send_token, self.approve_tools)
    ...
    async def run(self):
        ...
        try:
            async with UserAgent(self.session_id) as agent:
                ...
                while True:
                    question = await self.read_question()
                    if question is None:
                        await self.send_error("message is required")
                        continue
                    await self.answer(agent, question)
        except WebSocketDisconnect:
